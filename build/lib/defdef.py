...
class DefDefObj:
    ...
    funcspace_example = {
        "printTimes@File": {
            ...
            "func": lambda *args, Times=3, File="out.txt", **kwargs: (
                        print(*args, **kwargs, file=open(File, "a+"))
    ...
    def add(self, dict_or_func: "dict_func_type | namestr",*args):
        ...
        if isinstance(sth, (dict, OrderedDict)):
            ...
            assert len(sth)==1, "only one func can be added, you should ues list or tuple to add more at one time"
            if 'help' not in child :
                child['help'] = child['func'].__doc__
            if 'kwargs' not in child :
                child['kwargs'] = self.get_kwargs(child['func'])
            if 'splits' not in child :
                child['splits'] = self.get_splits(name,child['kwargs'])
            new_func={name:child}
    ...
    def func_find(self, name):
        ...
        for k, v in self.funcspace.items():
            ...
            if name == k:
                ...
                return k,self.namespace[name]['kwargs']
            else:
                kwargs =self.get_namespace_kwargs(k,child['kwargs'],child['splits'],name)
    ...
    @staticmethod
    def get_namespace_kwargs(funcspace_name,funcspace_kwargs,funcspace_splits,namespace_name):
    ...
    def get(self, func_name):
        ...
        if func_name in self.namespace:
            initkwargs = self.namespace[func_name]['kwargs']
