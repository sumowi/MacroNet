...
class Module(Module_base): # type: ignore
            
    # 定义一个函数，参数args为一个列表，call为一个函数
    def __init__(self,args=[],call=SEQ):
        # 调用父类的初始化函数
        super().__init__()
    ...
    def __add__(self,other):
        ...
        if isinstance(other,int):
            return self+[Module()]*other if other !=1 else self+[Module()]
        nn_list = [*self] if self.call == LIC else [self]
        nn_list += [*other] if (self.is_dup(other) and other.call != SEQ) or (hasattr(other,'__len__') and len(other)>=1) else [other]
    ...
    def __mul__(self,other):
        ...
        '''
        if isinstance(other,int):
            return Module([self]*other) if other !=1 else self
        else:
            nn_list = [*self] if self.call == SEQ else [self]
            nn_list += [*other] if (self.is_dup(other) and other.call != LIC) or (hasattr(other,'__len__') and len(other)>=1) else [other]
    ...
    def __pow__(self,other):
        ...
        import copy
        if isinstance(other,int):
            return Module([copy.deepcopy(self) for i in range(other)],call=SEQ) if other !=1 else copy.deepcopy(self)
        else:
    ...
    def __and__(self,other):
        ...
        import copy
        if isinstance(other,int):
            return Module([copy.deepcopy(self) for i in range(other)],call=LIC)+1 if other !=1 else copy.deepcopy(self)+1
        else:
